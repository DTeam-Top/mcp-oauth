...
export default async function AuthorizePage({
  ...
  const [client] = await database.select().from(clients).where(eq(clients.clientId, clientId));
  ...
  return (
    <main className="flex items-center justify-center h-screen bg-gray-50">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-sm w-full">
        <h1 className="text-xl font-semibold mb-4 text-center">Authorize Application</h1>
        <div className="text-center">
          <p className="mb-2">
            The application <strong className="font-medium">{client.name}</strong> is requesting
