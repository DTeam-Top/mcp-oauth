...
import { accessTokens, authCodes, clients } from '@/lib/db/schema';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [client] = await database.select().from(clients).where(eq(clients.clientId, client_id));
    if (!client) {
    ...
    const [authCode] = await database.select().from(authCodes).where(eq(authCodes.code, code));
    ...
    await database.delete(authCodes).where(eq(authCodes.id, authCode.id));
    ...
    await database.insert(accessTokens).values({
      token: accessToken,
      expiresAt,
      clientId: client.id,
